...
:class:`~loomworks.http.Controller`: class and its companion the
:func:`~loomworks.http.route`: method decorator. Together they are used to
...
def db_list(force=False, host=None):
    ...
    :param bool force: See :func:`~loomworks.service.db.list_dbs`:
    ...
    """
    try:
        dbs = loomworks.service.db.list_dbs(force)
...
def dispatch_rpc(service_name, method, params):
    ...
    """
    rpc_dispatchers = {
        'common': loomworks.service.common.dispatch,
        'db': loomworks.service.db.dispatch,
        'object': loomworks.service.model.dispatch,
    }
...
class Stream:
    ...
    def from_path(cls, path, filter_ext=('',), public=False):
        ...
        :param path: See :func:`~loomworks.tools.file_path`
        :param filter_ext: See :func:`~loomworks.tools.file_path`
    ...
    def get_response(
        ...
        **send_file_kwargs
        ...
        :param send_file_kwargs: Other keyword arguments to send to
            :func:`loomworks.tools._vendor.send_file.send_file` instead of
...
class Controller:
    ...
    :class:`~loomworks.http.Controller` can use the :func:`~loomworks.http.route`:
    ...
    :func:`~loomworks.http.route`:. Please note that the decorators of all
    ...
    .. code-block:
        ...
        class GreetingController(loomworks.http.Controller):
    ...
    def __init_subclass__(cls):
        ...
        if Controller in cls.__bases__:
            ...
            module = path[2] if path[:2] == ['loomworks', 'addons'] else ''
...
def route(route=None, **routing):
    ...
    .. warning::
        ...
        :class:`~loomworks.http.Controller` for more details.
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    ...
    def is_valid(cls):
        ...
        path = cls.__module__.split('.')
        return path[:2] == ['loomworks', 'addons'] and path[2] in modules
    ...
    def build_controllers():
        ...
        """
        # Controllers defined outside of loomworks addons are outside of the
        # controller inheritance/extension mechanism.
        yield from (ctrl() for ctrl in Controller.children_classes.get('', []))

        # Controllers defined inside of loomworks addons can be extended in
...
class Session(collections.abc.MutableMapping):
    ...
    def authenticate(self, dbname, credential):
        ...
        with registry.cursor() as cr:
            env = loomworks.api.Environment(cr, pre_uid, {})
        ...
        if request and request.session is self and request.db == dbname:
            request.env = loomworks.api.Environment(request.env.cr, self.uid, self.context)
...
class GeoIP(collections.abc.Mapping):
    ...
    .. note:
        ...
        :attr:`~loomworks.http.request.geoip`.
...
class _Response(werkzeug.wrappers.Response):
    ...
    def load(cls, result, fname="<function>"):
        ...
        :returns: The created :class:`~loomworks.http.Response`.
        :rtype: Response
...
class Response(Proxy):
    ...
    # loomworks.http._response attributes
    load = ProxyFunc()
...
class Request:
    ...
    def update_env(self, user=None, context=None, su=None):
        ...
        :type user: int or :class:`res.users record<~loomworks.addons.base.models.res_users.Users>`
    ...
    def make_response(self, data, headers=None, cookies=None, status=200):
        ...
        :rtype: :class:`~loomworks.http.Response`
        """
    ...
    def _serve_db(self):
        ...
        try:
            ...
            self.env = loomworks.api.Environment(cr_readwrite, self.session.uid, self.session.context)
            try:
            ...
            with contextlib.closing(self.registry.cursor(readonly=readonly_cr)) as cr:
                ...
                try:
                    ...
                    if isinstance(exc, HTTPException) and exc.code is None:
                        raise  # bubble up to loomworks.http.Application.__call__
...
class HttpDispatcher(Dispatcher):
    ...
    def dispatch(self, endpoint, args):
        ...
        See :meth:`~loomworks.http.Response.load` method for the compatible
...
class Application:
    ...
    def statics(self):
        ...
        """
        mod2path = {}
        for addons_path in loomworks.addons.__path__:
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint in _generate_routing_rules([''] + loomworks.conf.server_wide_modules, nodb_only=True):
    ...
    def session_store(self):
        path = loomworks.tools.config.session_dir
    ...
    def __call__(self, environ, start_response):
        ...
        if loomworks.tools.config['proxy_mode'] and environ.get("HTTP_X_FORWARDED_HOST"):
            ...
            try:
                ...
                if self.get_static_file(httprequest.path):
                    ...
                    try:
                        ...
                        if (httprequest.path.startswith('/loomworks/')
                            or httprequest.path in (
                                '/loomworks', '/web', '/web/login', '/test_http/ensure_db',
                            )):
