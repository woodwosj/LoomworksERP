...
import loomworks
import loomworks.tools as tools
import loomworks.release as release
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `loomworks.upgrade`"""

    def find_spec(self, fullname, path=None, target=None):
        if re.match(r"^loomworks\.addons\.base\.maintenance\.migrations\b", fullname):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("loomworks.addons.base.maintenance.migrations", "loomworks.upgrade")
...
def initialize_sys_path():
    """
    Setup the addons path ``loomworks.addons.__path__`` with various defaults
    and explicit directories.
    """
    # hook loomworks.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in loomworks.addons.__path__:
        loomworks.addons.__path__.append(dd)

    # hook loomworks.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(ad.strip()))
        if ad not in loomworks.addons.__path__:
            loomworks.addons.__path__.append(ad)

    # hook loomworks.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in loomworks.addons.__path__ and os.path.isdir(base_path):
        loomworks.addons.__path__.append(base_path)

    # hook loomworks.upgrade on upgrade-path
    from loomworks import upgrade
    ...
    # create decrecated module alias from loomworks.addons.base.maintenance.migrations to loomworks.upgrade
    spec = importlib.machinery.ModuleSpec("loomworks.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["loomworks.addons.base.maintenance"] = maintenance_pkg
    sys.modules["loomworks.addons.base.maintenance.migrations"] = upgrade

    # hook deprecated module alias from openerp to loomworks and "crm"-like to loomworks.addons
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    if re.search(r"[\/\\]", module):
        return False
    for adp in loomworks.addons.__path__:
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    sorted_paths = sorted(loomworks.addons.__path__, key=len, reverse=True)
...
def load_openerp_module(module_name):
    ...
    qualname = f'loomworks.addons.{module_name}'
...
def get_modules():
    ...
    plist = []
    for ad in loomworks.addons.__path__:
