...
def is_initialized(cr):
    ...
    """
    return loomworks.tools.sql.table_exists(cr, 'ir_module_module')
...
def initialize(cr):
    ...
    """
    try:
        f = loomworks.tools.misc.file_path('base/data/base_data.sql')
    except FileNotFoundError:
        m = "File not found: 'base.sql' (provided by module 'base')."
        _logger.critical(m)
        raise IOError(m)

    with loomworks.tools.misc.file_open(f) as base_sql_file:
        cr.execute(base_sql_file.read())  # pylint: disable=sql-injection

    for i in loomworks.modules.get_modules():
        mod_path = loomworks.modules.get_module_path(i)
        if not mod_path:
            continue

        # This will raise an exception if no/unreadable descriptor file.
        info = loomworks.modules.get_manifest(i)
    ...
    from loomworks.tools import config  # noqa: PLC0415
    while True:
