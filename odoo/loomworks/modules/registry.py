...
import loomworks
from loomworks.modules.db import FunctionStatus
from .. import SUPERUSER_ID
from loomworks.sql_db import TestCursor
from loomworks.tools import (
    config, lazy_classproperty,
    lazy_property, sql, OrderedSet, SQL,
    remove_accents,
)
from loomworks.tools.func import locked
from loomworks.tools.lru import LRU
from loomworks.tools.misc import Collector, format_frame
...
_schema = logging.getLogger('loomworks.schema')
...
class Registry(Mapping):
    ...
    def new(cls, db_name, force_demo=False, status=None, update_module=False):
        ...
        try:
            ...
            try:
                loomworks.modules.load_modules(registry, force_demo, status, update_module)
            except Exception:
                loomworks.modules.reset_modules_state(db_name)
                raise
        except Exception:
    ...
    def init(self, db_name):
        ...
        if config['test_enable'] or config['test_file']:
            from loomworks.tests.result import OdooTestResult  # noqa: PLC0415
        ...
        self._db = loomworks.sql_db.db_connect(db_name, readonly=False)
        ...
        if config['db_replica_host'] is not False or config['test_enable']:  # by default, only use readonly pool if we have a db_replica_host defined. Allows to have an empty replica host for testing
            self._db_readonly = loomworks.sql_db.db_connect(db_name, readonly=True)
        ...
        with closing(self.cursor()) as cr:
            self.has_unaccent = loomworks.modules.db.has_unaccent(cr)
            self.has_trigram = loomworks.modules.db.has_trigram(cr)
    ...
    def init_models(self, cr, model_names, context, install=True):
        ...
        env = loomworks.api.Environment(cr, SUPERUSER_ID, context)
    ...
    def cursor(self, /, readonly=False):
        ...
        if self.test_cr is not None:
            ...
            return TestCursor(self.test_cr, self.test_lock, readonly and self.test_readonly_enabled, current_test=loomworks.modules.module.current_test)
