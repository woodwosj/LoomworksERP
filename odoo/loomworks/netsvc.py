...
def log(logger, level, prefix, msg, depth=None):
    warnings.warn(
        "loomworks.netsvc.log is deprecated starting Odoo 18, use normal logging APIs",
...
class PostgreSQLHandler(logging.Handler):
    ...
    def __init__(self):
        ...
        if tools.config['log_db'] != '%d':
            with contextlib.suppress(Exception), tools.mute_logger('loomworks.sql_db'), sql_db.db_connect(tools.config['log_db'], allow_uri=True).cursor() as cr:
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('loomworks.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='loomworks.tools.image')
...
DEFAULT_LOG_CONFIGURATION = [
    'loomworks.http.rpc.request:INFO',
    'loomworks.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['loomworks:DEBUG', 'loomworks.sql_db:INFO', 'loomworks.http.rpc:DEBUG'],
    'debug_rpc': ['loomworks:DEBUG', 'loomworks.sql_db:INFO', 'loomworks.http.rpc.request:DEBUG'],
    'debug': ['loomworks:DEBUG', 'loomworks.sql_db:INFO'],
    'debug_sql': ['loomworks.sql_db:DEBUG'],
    'info': [],
    'runbot': ['loomworks:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['loomworks:WARNING', 'werkzeug:WARNING'],
    'error': ['loomworks:ERROR', 'werkzeug:ERROR'],
    'critical': ['loomworks:CRITICAL', 'werkzeug:CRITICAL'],
}
