...
from loomworks.tools import file_path, file_open, file_open_temporary_directory
from loomworks.tools.config import conf, configmanager, _get_default_datadir


IS_POSIX = 'workers' in loomworks.tools.config.options
ROOT_PATH = loomworks.tools.config.options['root_path'].removesuffix('/loomworks')
...
class TestConfigManager(TransactionCase):
    ...
    def setUp(self):
        ...
        self.patch(conf, 'addons_paths', loomworks.conf.addons_paths)
        self.patch(conf, 'server_wide_modules', loomworks.conf.server_wide_modules)
    ...
    def test_06_cli(self):
        ...
        values = {
            ...
            'log_handler': [
                ':INFO',
                'loomworks.tools.config:DEBUG',
                ':WARNING',
                'loomworks.http:DEBUG',
                'loomworks.sql_db:DEBUG',
            ],
            'log_db': 'logdb',
            'log_db_level': 'debug',
            'log_level': 'debug',
