...
from loomworks import release, SUPERUSER_ID, _
from loomworks.http import request
from loomworks.tools import (func, misc, transpile_javascript,
    is_odoo_module, SourceMapGenerator, profiler, OrderedSet)
from loomworks.tools.json import scriptsafe as json
from loomworks.tools.constants import SCRIPT_EXTENSIONS, STYLE_EXTENSIONS
from loomworks.tools.misc import file_open, file_path
        ...
        * 't-inherit-mode':  'primary' or 'extension'.
        ...
        for asset in self.templates:
            ...
            try:
                ...
                # Strip XML declaration if present before checking root tag
                check_content = content.split('\n', 1)[-1].strip() if content.startswith('<?xml') else content
                template = content if check_content.startswith(('<odoo>', '<loomworks>')) else f'<templates>{asset.content}</templates>'
