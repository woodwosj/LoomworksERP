...
from loomworks import api, fields, models, tools, SUPERUSER_ID, _, Command
from loomworks.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
from loomworks.exceptions import AccessDenied, AccessError, UserError, ValidationError
from loomworks.http import request, DEFAULT_LANG
from loomworks.osv import expression
from loomworks.tools import is_html_empty, partition, frozendict, lazy_property, SQL, SetDefinitions
...
class Users(models.Model):
    ...
    def _unlink_except_master_data(self):
        ...
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by Loomworks (updates, module installation, ...)'))
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: loomworks.exceptions.AccessDenied when old password is wrong
        :raise: loomworks.exceptions.UserError when new password is not set or empty
        """
        if not old_passwd:
            raise AccessDenied()
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "and *might* be a proxy. If your Loomworks ERP is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "Loomworks ERP in Proxy Mode and that the proxy is properly configured, see "
...
class GroupsImplied(models.Model):
    ...
    def _get_group_definitions(self):
        """ Return the definition of all the groups as a :class:`~loomworks.tools.SetDefinitions`. """
