...
import loomworks
from loomworks.http import Session
from loomworks.addons.base.tests.common import HttpCaseWithUserDemo
from loomworks.tools.func import lazy_property
from loomworks.addons.test_http.utils import MemoryGeoipResolver, MemorySessionStore
...
class TestHttpBase(HttpCaseWithUserDemo):
    ...
    def setUpClass(cls):
        ...
        cls.classPatch(loomworks.http.Application, 'session_store', session_store)
        cls.classPatch(loomworks.http.Application, 'geoip_city_db', geoip_resolver)
        cls.classPatch(loomworks.http.Application, 'geoip_country_db', geoip_resolver)

    def setUp(self):
        super().setUp()
        loomworks.http.root.session_store.store.clear()
    ...
    def nodb_url_open(self, url, *args, allow_redirects=False, **kwargs):
        with patch('loomworks.http.db_list') as db_list, \
             patch('loomworks.http.db_filter') as db_filter:
    ...
    def multidb_url_open(self, url, *args, allow_redirects=False, dblist=(), **kwargs):
        ...
        with patch('loomworks.http.db_list') as db_list, \
             patch('loomworks.http.db_filter') as db_filter, \
             patch('loomworks.http.Registry') as Registry:
