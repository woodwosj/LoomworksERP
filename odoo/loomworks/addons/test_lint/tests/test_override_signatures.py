...
from loomworks.tests.common import get_db_name, tagged
...
methods_to_sanitize = {
    method_name
    for method_name in dir(loomworks.models.BaseModel)
...
def get_odoo_module_name(python_module_name):
    if python_module_name.startswith('loomworks.addons.'):
        return python_module_name.split('.')[2]
    if python_module_name == 'loomworks.models':
        return 'odoo'
