...
import loomworks
from loomworks.modules import get_modules
from loomworks.modules.registry import Registry
from loomworks.release import nt_service_name
from loomworks.tools import config
from loomworks.tools.cache import log_ormcache_stats
from loomworks.tools.misc import stripped_sys_argv, dumpstacks
...
def set_limit_memory_hard():
    ...
    if loomworks.evented and config['limit_memory_hard_gevent']:
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    ...
    def setup(self):
        ...
        me.name = 'loomworks.service.http.request.%s' % (me.ident,)
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in loomworks.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="loomworks.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        from loomworks.addons.base.models.ir_cron import ir_cron
        def _run_cron(cr):
            pg_conn = cr._cnx
            ...
            while config['limit_time_worker_cron'] <= 0 or (time.monotonic() - alive_time) <= config['limit_time_worker_cron']:
                ...
                registries = loomworks.modules.registry.Registry.registries
        ...
        while True:
            conn = loomworks.sql_db.db_connect('postgres')
    ...
    def cron_spawn(self):
        ...
        for i in range(loomworks.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="loomworks.service.cron.cron%d" % i)
            t.daemon = True
            t.type = 'cron'
            t.start()
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                from loomworks.tests.result import _logger as logger  # noqa: PLC0415
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('loomworks: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(loomworks.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = loomworks.service.db.list_dbs(True)
        return db_names
    ...
    def process_work(self):
        ...
        if len(db_names):
            ...
            from loomworks.addons import base
    ...
    def start(self):
        ...
        dbconn = loomworks.sql_db.db_connect('postgres')
...
def _reexec(updated_modules=None):
    ...
    if loomworks.tools.osutil.is_running_as_nt_service():
...
def load_test_file_py(registry, test_file):
    ...
    from loomworks.tests import loader  # noqa: PLC0415
    from loomworks.tests.suite import OdooSuite  # noqa: PLC0415
    ...
    for dbname in dbnames:
        try:
            ...
            if config['test_enable']:
                from loomworks.tests import loader  # noqa: PLC0415
                t0 = time.time()
                t0_sql = loomworks.sql_db.sql_counter
                ...
                _logger.info("%d post-tests in %.2fs, %s queries",
                             ...
                             time.time() - t0,
                             loomworks.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    """ Start the loomworks http server and cron processor.
    """
    global server

    load_server_wide_modules()

    if loomworks.evented:
        server = GeventServer(loomworks.http.root)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(loomworks.http.root)
    else:
        ...
        server = ThreadedServer(loomworks.http.root)

    watcher = None
    if 'reload' in config['dev_mode'] and not loomworks.evented:
        if inotify:
