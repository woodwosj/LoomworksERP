...
import loomworks.sql_db
import loomworks.tools
from loomworks import SUPERUSER_ID
from loomworks.exceptions import AccessDenied
from loomworks.release import version_info
from loomworks.sql_db import db_connect
from loomworks.tools import SQL
from loomworks.tools.misc import exec_pg_environ, find_pg_tool
...
def check_db_management_enabled(method):
    def if_db_mgt_enabled(method, self, *args, **kwargs):
        if not loomworks.tools.config['list_db']:
...
def check_super(passwd):
    if passwd and loomworks.tools.config.verify_admin_password(passwd):
        return True
    raise loomworks.exceptions.AccessDenied()

# This should be moved to loomworks.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        db = loomworks.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by Registry.new().
            loomworks.modules.db.initialize(cr)
            loomworks.tools.config['load_language'] = lang
            cr.commit()

        registry = loomworks.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(registry.cursor()) as cr:
            env = loomworks.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = loomworks.tools.email_split(login)
                if emails:
...
def _check_faketime_mode(db_name):
    if os.getenv('ODOO_FAKETIME_TEST_MODE') and db_name in loomworks.tools.config['db_name'].split(','):
        try:
            db = loomworks.sql_db.db_connect(db_name)
...
def _create_empty_database(name):
    db = loomworks.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = loomworks.tools.config['db_template']
    ...
    try:
        db = loomworks.sql_db.db_connect(name)
        with db.cursor() as cr:
            cr.execute("CREATE EXTENSION IF NOT EXISTS pg_trgm")
            if loomworks.tools.config['unaccent']:
...
def exp_duplicate_database(db_original_name, db_name, neutralize_database=False):
    ...
    loomworks.sql_db.close_db(db_original_name)
    db = loomworks.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        # database-altering operations cannot be executed inside a transaction
        cr._cnx.autocommit = True
    ...
    registry = loomworks.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = loomworks.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)
        if neutralize_database:
            loomworks.modules.neutralize.neutralize_database(cr)

    from_fs = loomworks.tools.config.filestore(db_original_name)
    to_fs = loomworks.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    loomworks.modules.registry.Registry.delete(db_name)
    loomworks.sql_db.close_db(db_name)
    ...
    fs = loomworks.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        ...
        'version_info': loomworks.release.version_info,
        'major_version': loomworks.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            filestore = loomworks.tools.config.filestore(db_name)
            ...
            if stream:
                loomworks.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                loomworks.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
...
def restore_db(db, dump_file, copy=False, neutralize_database=False):
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
        ...
        registry = loomworks.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = loomworks.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    loomworks.modules.registry.Registry.delete(old_name)
    loomworks.sql_db.close_db(old_name)
    ...
    old_fs = loomworks.tools.config.filestore(old_name)
    new_fs = loomworks.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    loomworks.tools.config.set_admin_password(new_password)
    loomworks.tools.config.save(['admin_passwd'])
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        loomworks.tools.config['update']['base'] = True
        loomworks.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@loomworks.tools.mute_logger('loomworks.sql_db')
...
def list_dbs(force=False):
    if not loomworks.tools.config['list_db'] and not force:
        raise loomworks.exceptions.AccessDenied()

    if not loomworks.tools.config['dbfilter'] and loomworks.tools.config['db_name']:
        ...
        res = sorted(db.strip() for db in loomworks.tools.config['db_name'].split(','))
        return res
...
def list_db_incompatible(databases):
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if loomworks.tools.sql.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        loomworks.sql_db.close_db(database_name)
...
def exp_list_lang():
    return loomworks.tools.misc.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(loomworks.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()
...
def exp_server_version():
    ...
    """
    return loomworks.release.version
