...
import loomworks
import loomworks.addons
# get_encodings, ustr and exception_to_unicode were originally from tools.misc.
# There are moved to loglevels until we refactor tools.
from loomworks.loglevels import exception_to_unicode, get_encodings, ustr  # noqa: F401
...
if typing.TYPE_CHECKING:
    ...
    from loomworks.addons.base.models.res_lang import LangData
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if loomworks.tools.config['db_host']:
        env['PGHOST'] = loomworks.tools.config['db_host']
    if loomworks.tools.config['db_port']:
        env['PGPORT'] = str(loomworks.tools.config['db_port'])
    if loomworks.tools.config['db_user']:
        env['PGUSER'] = loomworks.tools.config['db_user']
    if loomworks.tools.config['db_password']:
        env['PGPASSWORD'] = loomworks.tools.config['db_password']
    return env
...
def file_path(file_path: str, filter_ext: tuple[str, ...] = ('',), env: Environment | None = None) -> str:
    ...
    addons_paths = [*loomworks.addons.__path__, root_path, *temporary_paths]
...
def file_open_temporary_directory(env: Environment):
    ...
    Examples::
        ...
        >>> with loomworks.tools.file_open_temporary_directory(self.env) as module_dir:
        ...    with zipfile.ZipFile('foo.zip', 'r') as z:
        ...        z.extract('foo/__manifest__.py', module_dir)
        ...    with loomworks.tools.file_open('foo/__manifest__.py', env=self.env) as f:
        ...        manifest = f.read()
...
class mute_logger(logging.Handler):
    ...
    Can be used as context manager or decorator::
        ...
        @mute_logger('loomworks.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('loomworks.foo.bar'):
            do_suff()
    """
...
def format_date(
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = loomworks.fields.Datetime.from_string(value)
            value = loomworks.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = loomworks.fields.Datetime.from_string(value)
...
def format_datetime(
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = loomworks.fields.Datetime.from_string(value)
    else:
        timestamp = value
...
def limited_field_access_token(record, field_name, timestamp=None):
    ...
    :type record: class:`loomworks.models.Model`
