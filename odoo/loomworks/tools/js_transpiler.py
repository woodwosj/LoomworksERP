"""
This code is what let us use ES6-style modules in Loomworks ERP.
Classic modules are composed of a top-level :samp:`odoo.define({name},{dependencies},{body_function})` call.
This processor will take files starting with an `@loomworks-module` annotation (in a comment) and convert them to classic modules.
If any file has the ``/** loomworks-module */`` on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual module format with minimal changes.
...
LOOMWORKS_MODULE_RE = re.compile(r"""
    \s*                                # starting white space
    \/(\*|\/)                          # /* or //
    .*                                 # any comment in between (optional)
    @loomworks-module                  # '@loomworks-module' statement
...
ODOO_MODULE_RE = LOOMWORKS_MODULE_RE


def is_odoo_module(url, content):
    """
    Detect if the file is a native loomworks module.
    We look for a comment containing @loomworks-module.

    :param url:
    :param content: source code
    :return: is this a loomworks module that needs transpilation?
    """
    result = LOOMWORKS_MODULE_RE.match(content)
...
def get_aliased_odoo_define_content(module_path, content):
    ...
    .. code-block:: javascript
        ...
        /** @loomworks-module */
        // after
        /** @loomworks-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @loomworks-module */
        // after
        /** @loomworks-module alias=web.AbstractAction default=false */

    :return: the alias content to append to the source code.
    """
    matchobj = LOOMWORKS_MODULE_RE.match(content)
    if matchobj:
