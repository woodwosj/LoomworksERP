...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", default=False,
                          help="save configuration to ~/.loomworksrc (or to ~/.openerp_serverrc if it exists)")
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", my_default=_get_default_datadir(),
                         help="Directory where to store Loomworks data")
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "loomworks.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="loomworks.http:DEBUG", help='shortcut for --log-handler=loomworks.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="loomworks.sql_db:DEBUG", help='shortcut for --log-handler=loomworks.sql_db:DEBUG')
    ...
    def parse_config(self, args: list[str] | None = None, *, setup_logging: bool | None = None) -> None:
        ...
        This method initializes loomworks.tools.config and openerp.conf (the
        ...
        Typical usage of this method:
            ...
            loomworks.tools.config.parse_config(sys.argv[1:])
        """
        opt = self._parse_config(args)
        if setup_logging is not False:
            loomworks.netsvc.init_logger()
        ...
        loomworks.modules.module.initialize_sys_path()
        return opt
    ...
    def _parse_config(self, args=None):
        ...
        # TODO use appdirs
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'loomworks.conf')
        else:
            rcfilepath = os.path.expanduser('~/.loomworksrc')
            old_rcfilepath = os.path.expanduser('~/.openerp_serverrc')
            old_odoorc = os.path.expanduser('~/.odoorc')

            die(os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath),
                "Found '.loomworksrc' and '.openerp_serverrc' in your path. Please keep only one of "\
                "them, preferably '.loomworksrc'.")

            # Backward compat: fall back to .odoorc if .loomworksrc doesn't exist
            if not os.path.isfile(rcfilepath) and os.path.isfile(old_odoorc):
                rcfilepath = old_odoorc

            if not os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath):
                rcfilepath = old_rcfilepath

        self.rcfile = os.path.abspath(
            self.config_file or opt.config or os.environ.get('LOOMWORKS_RC') or os.environ.get('ODOO_RC') or os.environ.get('OPENERP_SERVER') or rcfilepath)
        self.load()
    ...
    def _is_addons_path(self, path):
        from loomworks.modules.module import MANIFEST_NAMES
