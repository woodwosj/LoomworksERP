...
class Cloc(object):
    ...
    def count_path(self, path, exclude=None):
        path = path.rstrip('/')
        exclude_list = []
        for i in loomworks.modules.module.MANIFEST_NAMES:
    ...
    def count_modules(self, env):
        ...
        exclude_heuristic = [loomworks.modules.get_module_path(m, display_warning=False) for m in STANDARD_MODULES]
        ...
        for module_name in module_list:
            module_path = os.path.realpath(loomworks.modules.get_module_path(module_name))
            if module_path:
    ...
    def count_database(self, database):
        registry = loomworks.modules.registry.Registry(config['db_name'])
        with registry.cursor() as cr:
            uid = loomworks.SUPERUSER_ID
