...
__author__ = loomworks.release.author
__version__ = loomworks.release.version

# Also use the `loomworks` logger for the main script.
_logger = logging.getLogger('loomworks')
...
def report_configuration():
    ...
    _logger.info('addons paths: %s', loomworks.addons.__path__)
...
def setup_pid_file():
    ...
    """
    config = loomworks.tools.config
    if not loomworks.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = loomworks.modules.registry.Registry.new(dbname)
        with registry.cursor() as cr:
            loomworks.tools.translate.trans_export(config["language"],
...
def import_translation():
    ...
    registry = loomworks.modules.registry.Registry.new(dbname)
    with registry.cursor() as cr:
        translation_importer = loomworks.tools.translate.TranslationImporter(cr)
        translation_importer.load_file(config["translate_in"], config["language"])
        translation_importer.save(overwrite=overwrite)

def main(args):
    check_root_user()
    loomworks.tools.config.parse_config(args, setup_logging=True)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                loomworks.service.db._create_empty_database(db_name)
            ...
            except loomworks.service.db.DatabaseExists:
                pass
    ...
    setup_pid_file()
    rc = loomworks.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the odoo server (default command)"""
    def run(self, args):
        loomworks.tools.config.parser.prog = f'{Path(sys.argv[0]).name} {self.name}'
        main(args)
