...
import loomworks
from loomworks.modules import get_module_path, get_modules, initialize_sys_path
...
def main():
    ...
    if len(args) > 1 and args[0].startswith('--addons-path=') and not args[1].startswith("-"):
        ...
        loomworks.tools.config._parse_config([args[0]])
        args = args[1:]
    ...
    if len(args) and not args[0].startswith("-"):
        ...
        for module in get_modules():
            if (Path(get_module_path(module)) / 'cli').is_dir():
                try:
                    __import__('loomworks.addons.' + module)
