...
class Shell(Command):
    ...
    def init(self, args):
        ...
        loomworks.cli.server.report_configuration()
        loomworks.service.server.start(preload=[], stop=True)
    ...
    def shell(self, dbname):
        ...
        if dbname:
            ...
            with registry.cursor() as cr:
                ...
                ctx = loomworks.api.Environment(cr, uid, {})['res.users'].context_get()
                env = loomworks.api.Environment(cr, uid, ctx)
                local_vars['env'] = env
                ...
                # from loomworks.tests.shell.run_tests if the user hasn't done anything.
                cr.rollback()
