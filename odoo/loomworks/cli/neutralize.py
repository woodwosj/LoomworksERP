...
class Neutralize(Command):
    ...
    def run(self, args):
        parser = loomworks.tools.config.parser
        ...
        opt = loomworks.tools.config.parse_config(args, setup_logging=True)

        dbname = loomworks.tools.config['db_name']
        if not dbname:
        ...
        try:
            with loomworks.sql_db.db_connect(dbname).cursor() as cursor:
                if opt.to_stdout:
                    installed_modules = loomworks.modules.neutralize.get_installed_modules(cursor)
                    queries = loomworks.modules.neutralize.get_neutralization_queries(installed_modules)
                    ...
                    loomworks.modules.neutralize.neutralize_database(cursor)
