...
from loomworks.exceptions import CacheMiss
from loomworks.osv import expression

import typing
from loomworks.api import ContextType, DomainType, IdType, NewId, M, T
...
class Field(MetaField('DummyField', (object,), {}), typing.Generic[T]):
    ...
    :param str aggregator: aggregate function used by :meth:`~loomworks.models.Model.read_group`
    ...
    def mapped(self, records):
        ...
        over a simple call to `~loomworks.models.BaseModel.mapped()` on a recordset.
        """
        if self.name == 'id':
            # not stored in cache
...
class Float(Field[float]):
    ...
    :param digits: a pair (total, decimal) or a string referencing a
        :class:`~loomworks.addons.base.models.decimal_precision.DecimalPrecision` record name.
    ...
    :func:`~loomworks.fields.Float.round()` to round a float with the given precision.
    :func:`~loomworks.fields.Float.is_zero()` to check if a float equals zero at the given precision.
    :func:`~loomworks.fields.Float.compare()` to compare two floats at the given precision.
...
class Monetary(Field[float]):
    ...
    :class:`res_currency<loomworks.addons.base.models.res_currency.Currency>`.
    ...
    :param str currency_field: name of the :class:`Many2one` field
        holding the :class:`res_currency <loomworks.addons.base.models.res_currency.Currency>`
...
class Image(Binary):
    ...
    :param bool verify_resolution: whether the image resolution should be verified
        ...
        See :class:`loomworks.tools.image.ImageProcess` for maximum image resolution (default: ``50e6``).
...
class Many2one(_Relational[M]):
    ...
    :param bool check_company: Mark the field to be verified in
        :meth:`~loomworks.models.Model._check_company`. Has a different behaviour
...
class Properties(Field):
    ...
    current record. The container must have a :class:`~loomworks.fields.PropertiesDefinition`
...
class PropertiesDefinition(Field):
    """ Field used to define the properties definition (see :class:`~loomworks.fields.Properties`
...
class Command(enum.IntEnum):
    """
    :class:`~loomworks.fields.One2many` and :class:`~loomworks.fields.Many2many` fields
    ...
    def create(cls, values: dict):
        ...
        In case of a :class:`~loomworks.fields.Many2many` relation, one unique
        new record is created in the comodel such that all records in `self`
        are linked to the new record.

        In case of a :class:`~loomworks.fields.One2many` relation, one new record
    ...
    def delete(cls, id: int):
        ...
        In case of a :class:`~loomworks.fields.Many2many` relation, removing the
    ...
    def unlink(cls, id: int):
        ...
        In case of a :class:`~loomworks.fields.One2many` relation, the given record
...
class Many2many(_RelationalMulti[M]):
    ...
    :param bool check_company: Mark the field to be verified in
        :meth:`~loomworks.models.Model._check_company`. Add a default company
