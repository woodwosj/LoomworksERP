# -*- coding: utf-8 -*-
"""
The module :mod:`loomworks.tests.common` provides unittest test cases and a few
...
from loomworks.http import BadRequest
...
from loomworks.sql_db import BaseCursor, Cursor, TestCursor
from loomworks.tools import config, float_compare, mute_logger, profiler, SQL, DotDict
from loomworks.tools.mail import single_email_re
from loomworks.tools.misc import find_in_path, lower_logging
from loomworks.tools.xml_utils import _validate_xml
...
if loomworks.cli.COMMAND in ('server', 'start') and not (config['test_enable'] or config['test_file']):
    _logger.error(
...
def __getattr__(name):
    ...
    warnings.warn(
        "Since 18.0: loomworks.tests.common.Form is deprecated, use loomworks.tests.Form",
...
ADDONS_PATH = loomworks.tools.config['addons_path']
HOST = '127.0.0.1'
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = loomworks.SUPERUSER_ID
...
def get_db_name():
    db = loomworks.tools.config['db_name']
...
def standalone(*tags):
    ...
    def register(func):
        ...
        if func.__module__.startswith('loomworks.addons.'):
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('loomworks.addons.'):
...
class BaseCase(case.TestCase, metaclass=MetaCase):
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~loomworks.models.BaseModel`
        """
    ...
    def debug_mode(self):
        ...
        request = Mock(
            ...
            env=self.env,
            session=DotDict(loomworks.http.get_default_session(), debug='1'),
        )
        try:
            self.env.flush_all()
            ...
            popped_request = loomworks.http._request_stack.pop()
    ...
    def _patchExecute(self, actual_queries, flush=True):
        ...
        with (
            patch('loomworks.sql_db.Cursor.execute', execute),
        ...
        if self.warm:
            ...
            with patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/loomworks/addons/" in filename:
                        filename = filename.rsplit("/loomworks/addons/", 1)[1]
                    if count > expected:
    ...
    def assertRecordValues(
            self,
            records: loomworks.models.BaseModel,
    ...
    def check_test_cursor(self, operation):
        if loomworks.modules.module.current_test != self:
            message = f"Trying to open a test cursor for {self.canonical_tag} while already in a test {loomworks.modules.module.current_test.canonical_tag}"
            _logger.runbot(message)
            raise BadRequest(message)
        request = loomworks.http.request
    ...
    def get_method_additional_tags(self, test_method):
        ...
        """
        additional_tags = []
        if loomworks.tools.config['test_tags'] and 'is_query_count' in loomworks.tools.config['test_tags']:
...
class Approx:  # noqa: PLW1641
    ...
    """
    def __init__(self, value: float, rounding: int | float | loomworks.addons.base.models.res_currency.Currency, /, decorate: bool) -> None:  # noqa: PYI041
        self.value = value
...
class TransactionCase(BaseCase):
    ...
    cr: Cursor = None
    muted_registry_logger = mute_logger(loomworks.modules.registry._logger.name)
    freeze_time = None
    ...
    def _gc_filestore(cls):
        ...
        with Registry(get_db_name()).cursor() as cr:
            gc_env = api.Environment(cr, loomworks.SUPERUSER_ID, {})
    ...
    def setUpClass(cls):
        ...
        cls.env = api.Environment(cls.cr, loomworks.SUPERUSER_ID, {})
        ...
        cls._crypt_context_patcher = patch('loomworks.addons.base.models.res_users.Users._crypt_context', _crypt_context)
...
def save_test_file(test_name, content, prefix, extension='png', logger=_logger, document_type='Screenshot', date_format="%Y%m%d_%H%M%S_%f"):
    ...
    screenshots_dir = pathlib.Path(loomworks.tools.config['screenshots']) / get_db_name() / 'screenshots'
...
class HttpCase(TransactionCase):
    ...
    @classmethod
    def http_port(cls):
        if loomworks.service.server.server is None:
            return None
        return loomworks.service.server.server.httpd.server_port
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('loomworks.service.http.request.')]
        ...
        if request_threads:
            ...
            loomworks.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=keep_db)
        loomworks.http.root.session_store.save(self.session)

    def authenticate(self, user, password, *,
        browser: ChromeBrowser = None, session_extra: dict | None = None):
        if getattr(self, 'session', None):
            loomworks.http.root.session_store.delete(self.session)

        self.session = session = loomworks.http.root.session_store.new()
        session.update(
        ...
        if user: # if authenticated
            ...
            with patch('loomworks.addons.base.models.res_users.Users._check_credentials', new=patched_check_credentials):
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(_profiler, patch('loomworks.http.Request._get_profiler_context_manager', route_profiler))
    ...
    def get_method_additional_tags(self, test_method):
        ...
        if loomworks.tools.config['test_tags'] and 'is_tour' in loomworks.tools.config['test_tags']:
...
def tagged(*tags):
    ...
    By default, all Test classes from loomworks.tests.common have a test_tags
