...
import loomworks
from loomworks.tools import config, topological_sort, unique
from loomworks.modules.registry import Registry
from loomworks.netsvc import init_logger
from loomworks.tests import standalone_tests
import loomworks.tests.loader

_logger = logging.getLogger('loomworks.tests.test_module_operations')
...
def test_standalone(args):
    ...
    loomworks.service.db._check_faketime_mode(args.database)  # noqa: SLF001
    ...
    for module_name in registry._init_modules:
        ...
        loomworks.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        loomworks.tools.config['addons_path'] = ','.join([args.addons_path, loomworks.tools.config['addons_path']])
        if args.data_dir:
            loomworks.tools.config['data_dir'] = args.data_dir
    ...
    logging.config.dictConfig({
        ...
        'loggers': {
            'loomworks.modules.loading': {'level': 'CRITICAL'},
            'loomworks.sql_db': {'level': 'CRITICAL'},
            'loomworks.models.unlink': {'level': 'WARNING'},
            'loomworks.addons.base.models.ir_model': {'level': "WARNING"},
        }
    })
