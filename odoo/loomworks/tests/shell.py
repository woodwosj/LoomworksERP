...
def run_tests(env, test_tags, modules=None, reload_tests=False):
    ...
    if loomworks.cli.COMMAND != 'shell':
        _logger.error('run_tests should be used only in odoo shell')
        return

    if loomworks.tools.config['workers'] != 0:
        _logger.error('run_tests should be used only in threaded mode')
        return

    from loomworks.service.server import server  # noqa: PLC0415
    if not server.httpd:
    ...
    loomworks.tools.config['test_tags'] = test_tags
    loomworks.tools.config['test_enable'] = True
    report = _run_tests(env.cr.dbname, modules)
    loomworks.tools.config['test_enable'] = None
    loomworks.tools.config['test_tags'] = None
