...
describe("stateToUrl", () => {
    test("encodes URI compatible strings", (assert) => {
        expect(stateToUrl({})).toBe("/loomworks");
        expect(stateToUrl({ a: "11", b: "summer wine" })).toBe("/loomworks?a=11&b=summer%20wine");
        expect(stateToUrl({ b: "2", c: "", e: "kloug,gloubi" })).toBe(
            "/loomworks?b=2&c=&e=kloug%2Cgloubi"
        );
    });
    ...
    test("backwards compatibility: no action stack, action encoded in path", (assert) => {
        expect(stateToUrl({})).toBe("/loomworks");
        // action
        expect(stateToUrl({ action: "some-path" })).toBe("/loomworks/some-path");
        expect(stateToUrl({ active_id: 5, action: "some-path" })).toBe("/loomworks/5/some-path");
        expect(stateToUrl({ active_id: "some-active_id", action: "some-path" })).toBe(
            "/loomworks/some-path?active_id=some-active_id",
            { message: "only numeric active_id are encoded in path" }
        );
        expect(stateToUrl({ action: "some-path", resId: 2 })).toBe("/loomworks/some-path/2");
        expect(stateToUrl({ action: "some-path", resId: "some-resId" })).toBe(
            "/loomworks/some-path?resId=some-resId",
            { message: "only numeric resId are encoded in path" }
        );
        expect(stateToUrl({ active_id: 5, action: "some-path", resId: 2 })).toBe(
            "/loomworks/5/some-path/2"
        );
        expect(stateToUrl({ active_id: 5, action: "some-path", resId: "new" })).toBe(
            "/loomworks/5/some-path/new"
        );
        expect(stateToUrl({ action: 1, resId: 2 })).toBe("/loomworks/action-1/2", {
            message: "action id instead of path/tag",
        });
        expect(stateToUrl({ action: "module.xml_id", resId: 2 })).toBe(
            "/loomworks/action-module.xml_id/2",
            { message: "action xml_id instead of path/tag" }
        );
        // model
        expect(stateToUrl({ model: "some.model" })).toBe("/loomworks/some.model");
        expect(stateToUrl({ model: "some.model", resId: 2 })).toBe("/loomworks/some.model/2");
        expect(stateToUrl({ active_id: 5, model: "some.model" })).toBe("/loomworks/5/some.model");
        expect(stateToUrl({ active_id: 5, model: "some.model", resId: 2 })).toBe(
            "/loomworks/5/some.model/2"
        );
        expect(stateToUrl({ active_id: 5, model: "some.model", resId: "new" })).toBe(
            "/loomworks/5/some.model/new"
        );
        expect(stateToUrl({ active_id: 5, model: "some.model", view_type: "some_viewtype" })).toBe(
            "/loomworks/5/some.model?view_type=some_viewtype"
        );
        // edge cases
        expect(
            stateToUrl({ active_id: 5, action: "some-path", resId: 2, some_key: "some_value" })
        ).toBe("/loomworks/5/some-path/2?some_key=some_value", {
        ...
        ).toBe("/loomworks/5/some-path/2", { message: "action has priority on model" });
        expect(stateToUrl({ active_id: 5, model: "some.model", resId: 2, view_type: "list" })).toBe(
            "/loomworks/5/some.model/2?view_type=list",
    ...
    test("actionStack: one action", () => {
        expect(stateToUrl({ actionStack: [] })).toBe("/loomworks");
        // action
        expect(stateToUrl({ actionStack: [{ action: "some-path" }] })).toBe("/loomworks/some-path");
        expect(stateToUrl({ actionStack: [{ active_id: 5, action: "some-path" }] })).toBe(
            "/loomworks/5/some-path"
        );
        expect(stateToUrl({ actionStack: [{ action: "some-path", resId: 2 }] })).toBe(
            "/loomworks/some-path/2"
        );
        ...
        ).toBe("/loomworks/5/some-path/new");
        expect(stateToUrl({ actionStack: [{ action: 1, resId: 2 }] })).toBe("/loomworks/action-1/2", {
    ...
    test("actionStack: multiple actions", () => {
        ...
        ).toBe("/loomworks/some-path/other-path");
        expect(
            stateToUrl({
                actionStack: [{ active_id: 5, action: "some-path" }, { action: "other-path" }],
            })
        ).toBe("/loomworks/5/some-path/other-path");
        expect(
            stateToUrl({
            ...
            "/loomworks/some-path/7/other-path"
        );
        expect(
            stateToUrl({
            ...
            "/loomworks/some-path/2/other-path"
        );
        expect(
            stateToUrl({
            ...
            "/loomworks/some-path/other-path/2"
        );
        expect(
            stateToUrl({
                actionStack: [
        ...
        ).toBe("/loomworks/5/some-path/2/other-path");
        expect(
            stateToUrl({
                actionStack: [
                    { action: "some-path" },
        ...
        ).toBe("/loomworks/some-path/5/other-path/2");
        expect(
            stateToUrl({
                actionStack: [
                    { action: "some-path" },
        ...
        ).toBe("/loomworks/some-path/5/other-path/new");
        expect(
            stateToUrl({
                actionStack: [
        ...
        ).toBe("/loomworks/some-path/5/other-path", {
            message:
        ...
        ).toBe("/loomworks/action-1/5/action-6/2", { message: "numerical actions" });
        expect(
            stateToUrl({
                actionStack: [
        ...
        ).toBe("/loomworks/action-module.xml_id/5/action-module.other_xml_id/2", {
            message: "actions as xml_ids",
        });
        // same action twice
        expect(
            stateToUrl({ actionStack: [{ action: "some-path" }, { action: "some-path" }] })
        ).toBe("/loomworks/some-path", {
        ...
        ).toBe("/loomworks/5/some-path/some-path", {
        ...
        ).toBe("/loomworks/some-path/7/some-path", {
            message:
        ...
        ).toBe("/loomworks/some-path/2/some-path");
        expect(
            stateToUrl({
                actionStack: [
        ...
        ).toBe("/loomworks/7/some-path/7/some-path", {
            message:
        ...
        ).toBe("/loomworks/5/some-path/2/some-path");
        expect(
            stateToUrl({
                actionStack: [
                    { action: "some-path" },
        ...
        ).toBe("/loomworks/some-path/5/some-path/2", {
            message:
        ...
        ).toBe("/loomworks/some-path/5/some-path/new");
        expect(
            stateToUrl({
                actionStack: [
                    { action: "some-path", resId: 5 },
        ...
        ).toBe("/loomworks/some-path/5/some-path", {
            message:
                "action with resId followed by action with same value as active_id is not duplicated",
        });
        expect(
        ...
        ).toBe("/loomworks/5/some-path/5/some-path/2", {
        ...
        ).toBe("/loomworks/action-1/5/action-1/2", { message: "numerical actions" });
        expect(
            stateToUrl({
                actionStack: [
                    { action: "module.xml_id" },
        ...
        ).toBe("/loomworks/5/some.model/2/other.model");
        expect(
            stateToUrl({
                actionStack: [
                    { model: "some.model" },
        ...
        ).toBe("/loomworks/some.model/5/other.model/2");
        expect(
            stateToUrl({
                actionStack: [
                    { model: "some.model" },
        ...
        ).toBe("/loomworks/some.model/5/other.model/new");
        expect(
            stateToUrl({
                actionStack: [
        ...
        ).toBe("/loomworks/some.model/5/other.model", {
            message:
                "action with resId followed by action with same value as active_id is not duplicated",
        });
        expect(
        ...
        ).toBe("/loomworks/5/some-path/2/some.model");
        expect(
            stateToUrl({
                actionStack: [
                    { action: "some-path" },
        ...
        ).toBe("/loomworks/some-path/5/some.model/2");
        expect(
            stateToUrl({
                actionStack: [
                    { action: "some-path" },
        ...
        ).toBe("/loomworks/some-path/5/some.model/new");
        expect(
            stateToUrl({
                actionStack: [
                    { action: "some-path", resId: 5 },
        ...
        ).toBe("/loomworks/some-path/5/some.model", {
            message:
                "action with resId followed by action with same value as active_id is not duplicated",
        });
        expect(
        ...
        ).toBe("/loomworks/action-1/5/m-model_no_dot/2");
        expect(
            stateToUrl({
                actionStack: [
        ...
        ).toBe("/loomworks/5/some.model/2/other-path");
        expect(
            stateToUrl({
                actionStack: [
                    { model: "some.model" },
        ...
        ).toBe("/loomworks/some.model/5/other-path/2");
        expect(
            stateToUrl({
                actionStack: [
                    { model: "some.model" },
        ...
        ).toBe("/loomworks/some.model/5/other-path/new");
        expect(
            stateToUrl({
                actionStack: [
                    { model: "some.model", resId: 5 },
        ...
        ).toBe("/loomworks/some.model/5/other-path", {
            message:
                "action with resId followed by action with same value as active_id is not duplicated",
        });
        expect(
        ...
        ).toBe("/loomworks/m-model_no_dot/5/action-1/2");
        expect(
            stateToUrl({
                actionStack: [
                    { model: "model_no_dot", resId: 5 },
        ...
        ).toBe("/loomworks/m-model_no_dot/5/action-module.xml_id/2");
        ...
        ).toBe("/loomworks/some-path/5/other-path/2?some_key=some_value", {
            message:
                "pieces of state unrelated to actions are added as query string even with actionStack",
        });
        expect(
        ...
        ).toBe("/loomworks/some-path/5/other-path/2", { message: "action has priority on model" });
        expect(
            stateToUrl({
                actionStack: [
                    { action: "some-path", resId: 5 },
        ...
        ).toBe("/loomworks/some-path/5/some.model/2?view_type=list", {
            message: "view_type and resId aren't incompatible",
        });
        expect(
            stateToUrl({
        ...
        ).toBe("/loomworks/some-path/2/5/other-path", {
            message:
        ...
        ).toBe("/loomworks/some.model/2/5/other.model", {
            message:
                "action with resId followed by action with different active_id gets both ids in a row",
        });
        expect(
        ...
        ).toBe("/loomworks/other-path/5/some-path/2", {
            message:
...
describe("urlToState", () => {
    test("deserialize queryString", () => {
        expect(_urlToState("/loomworks?a=11&g=summer%20wine")).toEqual({
            a: 11,
            g: "summer wine",
        });
        expect(_urlToState("/loomworks?g=2&c=&e=kloug%2Cgloubi")).toEqual({
            g: 2,
            c: "",
            e: "kloug,gloubi",
        });
    ...
    test("deserialize single action", () => {
        ...
        expect(_urlToState("/loomworks")).toEqual({});
        // action
        expect(_urlToState("/loomworks/some-path")).toEqual({
            action: "some-path",
            actionStack: [{ action: "some-path" }],
        });
        expect(_urlToState("/loomworks/5/some-path")).toEqual({
            active_id: 5,
            action: "some-path",
            actionStack: [{ active_id: 5, action: "some-path" }],
        });
        expect(_urlToState("/loomworks/some-path/2")).toEqual(
            {
                action: "some-path",
                resId: 2,
                actionStack: [{ action: "some-path" }, { action: "some-path", resId: 2 }],
        ...
        expect(_urlToState("/loomworks/some-path/new")).toEqual(
            {
                action: "some-path",
                resId: "new",
        ...
        expect(_urlToState("/loomworks/5/some-path/2")).toEqual({
            active_id: 5,
            action: "some-path",
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/loomworks/action-1/2")).toEqual({
            action: 1,
            resId: 2,
            actionStack: [{ action: 1 }, { action: 1, resId: 2 }],
        });
        expect(_urlToState("/loomworks/action-module.xml_id/2")).toEqual({
            action: "module.xml_id",
            resId: 2,
            actionStack: [{ action: "module.xml_id" }, { action: "module.xml_id", resId: 2 }],
        });
        // model
        expect(_urlToState("/loomworks/some.model")).toEqual({
            model: "some.model",
            actionStack: [{ model: "some.model" }],
        });
        expect(_urlToState("/loomworks/some.model/2")).toEqual(
            {
                model: "some.model",
                resId: 2,
        ...
        expect(_urlToState("/loomworks/some.model/new")).toEqual(
            {
                model: "some.model",
                resId: "new",
        ...
        expect(_urlToState("/loomworks/5/some.model")).toEqual({
            active_id: 5,
            model: "some.model",
            actionStack: [{ active_id: 5, model: "some.model" }],
        });
        expect(_urlToState("/loomworks/5/some.model/2")).toEqual({
            active_id: 5,
            model: "some.model",
            resId: 2,
            actionStack: [{ active_id: 5, model: "some.model", resId: 2 }],
        });
        expect(_urlToState("/loomworks/5/some.model?view_type=some_viewtype")).toEqual(
            {
                active_id: 5,
                model: "some.model",
                view_type: "some_viewtype",
        ...
        expect(_urlToState("/loomworks/m-model_no_dot/2")).toEqual({
            model: "model_no_dot",
            resId: 2,
            actionStack: [{ model: "model_no_dot", resId: 2 }],
        });
        // edge cases
        expect(_urlToState("/loomworks/5/some-path/2?some_key=some_value")).toEqual(
            {
                active_id: 5,
                action: "some-path",
                resId: 2,
        ...
        expect(_urlToState("/loomworks/5/some.model/2?view_type=list")).toEqual(
            {
                active_id: 5,
                model: "some.model",
                resId: 2,
    ...
    test("deserialize multiple actions", () => {
        // action
        expect(_urlToState("/loomworks/some-path/other-path")).toEqual({
            action: "other-path",
            actionStack: [{ action: "some-path" }, { action: "other-path" }],
        });
        expect(_urlToState("/loomworks/5/some-path/other-path")).toEqual({
            action: "other-path",
            actionStack: [{ active_id: 5, action: "some-path" }, { action: "other-path" }],
        });
        expect(_urlToState("/loomworks/some-path/2/other-path")).toEqual({
            action: "other-path",
            active_id: 2,
            actionStack: [
                { action: "some-path" },
        ...
        expect(_urlToState("/loomworks/some-path/other-path/2")).toEqual({
            action: "other-path",
            resId: 2,
            actionStack: [
                { action: "some-path" },
        ...
        expect(_urlToState("/loomworks/5/some-path/2/other-path")).toEqual({
            action: "other-path",
            active_id: 2,
            actionStack: [
                { active_id: 5, action: "some-path" },
        ...
        expect(_urlToState("/loomworks/some-path/5/other-path/2")).toEqual({
            active_id: 5,
            action: "other-path",
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/loomworks/some-path/5/other-path/new")).toEqual({
            active_id: 5,
            action: "other-path",
            resId: "new",
            actionStack: [
        ...
        expect(_urlToState("/loomworks/action-1/5/action-6/2")).toEqual({
            active_id: 5,
            action: 6,
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/loomworks/action-module.xml_id/5/action-module.other_xml_id/2")).toEqual({
            active_id: 5,
        ...
        // model
        expect(_urlToState("/loomworks/some.model/other.model")).toEqual(
            {
                model: "other.model",
        ...
        expect(_urlToState("/loomworks/5/some.model/other.model")).toEqual(
            {
                model: "other.model",
                actionStack: [{ model: "other.model" }],
            },
        ...
        expect(_urlToState("/loomworks/some.model/7/other.model")).toEqual({
            active_id: 7,
            model: "other.model",
            actionStack: [
        ...
        expect(_urlToState("/loomworks/some.model/other.model/2")).toEqual({
            model: "other.model",
            resId: 2,
            actionStack: [{ model: "other.model", resId: 2 }],
        });
        expect(_urlToState("/loomworks/5/some.model/2/other.model")).toEqual({
            active_id: 2,
            model: "other.model",
            actionStack: [
                { active_id: 5, model: "some.model", resId: 2 },
        ...
        expect(_urlToState("/loomworks/some.model/5/other.model/2")).toEqual({
            active_id: 5,
            model: "other.model",
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/loomworks/some.model/5/other.model/new")).toEqual({
            active_id: 5,
            model: "other.model",
            resId: "new",
            actionStack: [
        ...
        expect(_urlToState("/loomworks/m-model_no_dot/5/m-no_dot_model/2")).toEqual({
            active_id: 5,
            model: "no_dot_model",
            resId: 2,
            actionStack: [
        ...
        // action + model
        expect(_urlToState("/loomworks/some-path/some.model")).toEqual({
            model: "some.model",
            actionStack: [{ action: "some-path" }, { model: "some.model" }],
        });
        expect(_urlToState("/loomworks/5/some-path/some.model")).toEqual({
            model: "some.model",
            actionStack: [{ active_id: 5, action: "some-path" }, { model: "some.model" }],
        });
        expect(_urlToState("/loomworks/some-path/7/some.model")).toEqual({
            active_id: 7,
            model: "some.model",
            actionStack: [
                { action: "some-path" },
        ...
        expect(_urlToState("/loomworks/some-path/some.model/2")).toEqual({
            model: "some.model",
            resId: 2,
            actionStack: [{ action: "some-path" }, { model: "some.model", resId: 2 }],
        });
        expect(_urlToState("/loomworks/5/some-path/2/some.model")).toEqual({
            active_id: 2,
            model: "some.model",
            actionStack: [
                { active_id: 5, action: "some-path" },
        ...
        expect(_urlToState("/loomworks/some-path/5/some.model/2")).toEqual({
            active_id: 5,
            model: "some.model",
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/loomworks/some-path/5/some.model/new")).toEqual({
            active_id: 5,
            model: "some.model",
            resId: "new",
            actionStack: [
        ...
        expect(_urlToState("/loomworks/action-1/5/m-model_no_dot/2")).toEqual({
            active_id: 5,
            model: "model_no_dot",
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/loomworks/action-module.xml_id/5/m-model_no_dot/2")).toEqual({
            active_id: 5,
            model: "model_no_dot",
            resId: 2,
            actionStack: [
        ...
        // model + action
        expect(_urlToState("/loomworks/some.model/other-path")).toEqual({
            action: "other-path",
            actionStack: [{ action: "other-path" }],
        });
        expect(_urlToState("/loomworks/5/some.model/other-path")).toEqual({
            action: "other-path",
            actionStack: [{ action: "other-path" }],
        });
        expect(_urlToState("/loomworks/some.model/2/other-path")).toEqual({
            active_id: 2,
            action: "other-path",
            actionStack: [
                { model: "some.model", resId: 2 },
        ...
        expect(_urlToState("/loomworks/some.model/other-path/2")).toEqual({
            action: "other-path",
            resId: 2,
            actionStack: [{ action: "other-path" }, { action: "other-path", resId: 2 }],
        });
        expect(_urlToState("/loomworks/5/some.model/2/other-path")).toEqual({
            active_id: 2,
            action: "other-path",
            actionStack: [
                { active_id: 5, model: "some.model", resId: 2 },
        ...
        expect(_urlToState("/loomworks/some.model/5/other-path/2")).toEqual({
            active_id: 5,
            action: "other-path",
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/loomworks/some.model/5/other-path/new")).toEqual({
            active_id: 5,
            action: "other-path",
            resId: "new",
            actionStack: [
        ...
        expect(_urlToState("/loomworks/m-model_no_dot/5/action-1/2")).toEqual({
            active_id: 5,
            action: 1,
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/loomworks/m-model_no_dot/5/action-module.xml_id/2")).toEqual({
            active_id: 5,
            action: "module.xml_id",
            resId: 2,
            actionStack: [
        ...
        // edge cases
        expect(_urlToState("/loomworks/some-path/5/other-path/2?some_key=some_value")).toEqual({
            active_id: 5,
            action: "other-path",
            resId: 2,
            actionStack: [
        ...
        expect(_urlToState("/loomworks/some-path/5/some.model?view_type=list")).toEqual(
            {
                active_id: 5,
                model: "some.model",
                actionStack: [
        ...
        expect(_urlToState("/loomworks/some-path/5/some.model/2?view_type=list")).toEqual(
            {
                active_id: 5,
                model: "some.model",
                resId: 2,
        ...
        expect(_urlToState("/loomworks/some-path/2/5/other-path")).toEqual(
            {
                active_id: 5,
                action: "other-path",
                actionStack: [
        ...
        expect(_urlToState("/loomworks/some.model/2/5/other.model")).toEqual(
            {
                active_id: 5,
                model: "other.model",
                actionStack: [
...
describe("Retrocompatibility", () => {
    test("parse an url with hash (key/values)", async () => {
        ...
        expect(browser.location.pathname).toBe("/loomworks");
    });
    ...
    test("parse an url with hash (anchor link) and query string", async () => {
        redirect("/loomworks?a=114&g=c.e&f=1#anchor");
        browser.location.hash = "#anchor";
...
describe("internal links", () => {
    test("click on internal link does a loadState instead of a full reload", async () => {
        ...
        link.href = "/loomworks/some-action/2";
    ...
    test("click on internal link with different protocol does a loadState", async () => {
        ...
        const fixture = getFixture();
        const link = document.createElement("a");
        link.href = "http://" + browser.location.host + "/loomworks/some-action/2";
        fixture.appendChild(link);
    ...
    test("click on internal link with hash (key/values)", async () => {
        ...
        const fixture = getFixture();
        const link = document.createElement("a");
        link.href = "/loomworks/1/action-114/22";
        fixture.appendChild(link);
    ...
    test("click on internal link with hash (anchor)", async () => {
        ...
        const fixture = getFixture();
        const link = document.createElement("a");
        link.href = "/loomworks/1/action-114/22#anchorId";
        fixture.appendChild(link);
